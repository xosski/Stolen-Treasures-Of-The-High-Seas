...
def fetch_html_for_policy_check(url: str, extra_headers: dict[str, str], timeout: float) -> tuple[int, str, str, str]:
...
def write_csv(path: Path, rows: list[dict]) -> None:
    ...
    # union of all keys so metadata and findings can share one CSV
    keys = []
    for row in rows:
        for k in row.keys():
            if k not in keys:
